...
| BRD-REQ-016 | CRUD clientes | SPR-B003, SPR-F003 | PENDING_SPR-B003_COMMIT_HASH; PENDING_SPR-F003_COMMIT_HASH | `./mvnw test` + smoke + `npm run build` | READY_FOR_VALIDATION | Front CRM implementa crear/editar/listar/ficha cliente con contratos reales branch-scoped. |
| BRD-REQ-017 | Búsqueda clientes | SPR-B003, SPR-F003 | PENDING_SPR-B003_COMMIT_HASH; PENDING_SPR-F003_COMMIT_HASH | manual + smoke + `npm run build` | READY_FOR_VALIDATION | Ruta `/clientes` consume `GET /api/v1/clients?q=` con búsqueda por nombre/teléfono/identificación. |
| BRD-REQ-018 | CRUD mascotas | SPR-B003, SPR-F003 | PENDING_SPR-B003_COMMIT_HASH; PENDING_SPR-F003_COMMIT_HASH | `./mvnw test` + smoke + `npm run build` | READY_FOR_VALIDATION | Ficha `/clientes/[clientId]` integra alta/edición/listado de mascotas con endpoints reales de B003. |
| BRD-REQ-019 | Código interno mascota | SPR-B003, SPR-F003 | PENDING_SPR-B003_COMMIT_HASH; PENDING_SPR-F003_COMMIT_HASH | tests+manual + `npm run build` | READY_FOR_VALIDATION | UI muestra conflicto humano ante `PET_INTERNAL_CODE_CONFLICT` y mantiene validación primaria en backend. |
| BRD-REQ-020 | 1 mascota = 1 propietario | SPR-B003, SPR-F003 | PENDING_SPR-B003_COMMIT_HASH; PENDING_SPR-F003_COMMIT_HASH | manual + `npm run build` | READY_FOR_VALIDATION | Alta/edición de mascota se realiza dentro de cliente fijo; UI no permite cambiar propietario en v1. |
